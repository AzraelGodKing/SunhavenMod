...
- **2026-04-16** — Updated the release workflow build job to Variant A self-hosted execution (`runs-on: [self-hosted, windows, sunhaven-mods]`) so .NET mod builds run on your own Windows PC runner.
...
- **2026-04-16** — CI release pipeline now builds selected mods on a Windows GitHub runner using `.NET` (`actions/setup-dotnet`) and uploads DLL artifacts that the packaging/release job consumes, removing the need to commit pre-built DLLs before dispatching [`.github/workflows/build-release-publish.yml`](.github/workflows/build-release-publish.yml).
