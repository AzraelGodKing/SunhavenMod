...
- **Field highlights:** `Assets/tile_selection_sheet.png` (3× 36px sprites in one row) via `TileSelectionAssetLoader`; yellow 9-slice corners for dry, green 9-slice corners for unfertilized; probes vanilla sprite border/ppu when available. **Fix:** `CropSceneCache` no longer clears forecast on failed type lookup or backfills bogus ETA/gold; `GameSelectionHighlightRenderer` retries prototype resolution after early-session misses.
