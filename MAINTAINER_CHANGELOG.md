...
- **CharacterSaveStore — load failures:** `LoadWithBackup` wraps `tryDeserialize` in try/catch and forwards exceptions via `onReadFailure`; added unit test. Fixed `WriteAtomicCore` finally so failed writes delete `.tmp` when `deleteTempInFinally` is true (regression from merged `#90`).
