...
namespace GiftingAssistant.UI
{
    ...
    {
        private const float BgWidth = 8f;
        private const float BgHeight = 7f;
        private const float IconWidth = 12f;
        private const float IconHeight = 10f;
        private const float FillWidth = 12f;
        private const float FillHeight = 10f;
        private const float Gutter = 2f;

        public static float GridWidth(int maxHearts, float scale, bool singleRow = false)
        {
            int cols = singleRow ? maxHearts : Mathf.Min(RelationshipHeartRules.HeartsPerRow, maxHearts);
            float cellW = ScaledIconWidth(scale);
            return cols * cellW + Mathf.Max(0, cols - 1) * Scaled(Gutter, scale);
        }

        public static float GridHeight(int maxHearts, float scale, bool singleRow = false)
        {
            if (singleRow)
                return ScaledIconHeight(scale);
        ...
        public static void DrawGrid(Rect origin, float points, bool romanceable, float scale, bool singleRow = false)
        {
