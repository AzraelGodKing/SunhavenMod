...
- **versions.json:** Added **Gifting Assistant** **1.0.0** release metadata (`com.azraelgodking.giftingassistant`); Thunderstore and Nexus URLs left empty until first publish. Aligned `versions.json` changelog and mod README with the full 1.0.0 feature set (Todo PushToTodo default, Almanac, preferred gifts, hearts, Ctrl+G toggle).
